...
| `Praxy:Sites:KeepDeploymentImages` / `Praxy:Functions:KeepDeploymentImages` | `5` / `5` | How many of a site's/function's most recent successful builds keep their Docker image on disk. The active deployment's image is always kept on top of this, whatever its age. Everything older is reclaimed on the retention interval, which retires those deployments' rollback (and, for a site, their preview URL) — see [Disk usage: Docker images and build cache](#disk-usage-docker-images-and-build-cache). |
...
## Disk usage: Docker images and build cache

Files in Postgres are not the only way a Praxy instance uses disk. Building sites and functions
produces Docker images and Docker build cache, and the two are managed very differently: **Praxy
bounds the images it builds; it does not touch build cache, and you should.**

### Deployment images — bounded automatically

Every site and function deployment builds an image tagged `praxy-site-<deploymentId>` or
`praxy-fn-<deploymentId>`. A built Next.js site runs about 300 MB, a function about 235 MB. A site
wired to push-to-deploy builds one per commit.

`DeploymentImageSweeper` runs on the retention interval and keeps, **per site and per function**:

- the **active deployment's** image, whatever its age — rolling back to an old build and staying
  there never puts the running deployment at risk; and
- the **most recent successful builds**, bounded by `Praxy:Sites:KeepDeploymentImages` and
  `Praxy:Functions:KeepDeploymentImages` (5 each by default).

Everything older is removed, along with any image whose deployment (or whose whole site or function)
has been deleted.

The window exists because **an old deployment image is what rollback rolls back to.** Keeping only
the running image would make the console's "Activate" button — the thing that makes a bad deploy
survivable — a one-way door. Set the knob to the number of bad deploys you want to be able to walk
back, not to zero.

Reclaiming an image is visible rather than silent. The deployment row survives with its build log
and commit intact, the console marks it **image reclaimed**, and its Activate button is disabled;
activating it over the API returns `site_deployment_image_reclaimed` /
`function_deployment_image_reclaimed`. For a **site, this also retires that deployment's preview
URL**, since previews are cold-started from that same image. Redeploy the commit to get a fresh
image.

### Build cache — yours to bound

Docker's build cache is a separate pool from images, and **nothing in Praxy prunes it.** That is
deliberate on two counts: the cache is mostly generated by building *Praxy's own `api` image* during
an upgrade, which is the layer that builds the product rather than something the product builds; and
the pinned Docker client (`Docker.DotNet.Enhanced`) exposes no build-cache prune API at all — it can
only report the total, which is what `docker system df` already does.

It is not small. Measured on the praxycore.dev droplet, **one `docker compose up -d --build` left
4.63 GB of build cache**, 3.55 GB of it immediately reclaimable — a 2.18 GB `dotnet publish` mount,
the .NET SDK and ASP.NET runtime layers, and the console's `npm ci`. Each upgrade replaces that
generation without removing the previous one. Left alone on a 77 GB disk this reached **42 GB**, and
a full disk is not a graceful failure: Linux remounts the filesystem read-only, and Postgres starts
