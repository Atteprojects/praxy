...
/// <summary>
/// Deletes rows past their configured age from the three tables that otherwise grow forever:
...
/// mapping), and <c>praxy.audit_log</c>. Same shape as <c>FunctionPoolSweeper</c>: a loop, a
/// try/catch around the actual work that logs and continues rather than crashing the host, then
/// <see cref="Task.Delay(TimeSpan, CancellationToken)"/> on a configurable interval.
/// </summary>
    ...
    ILogger<RetentionSweeper> logger) : BackgroundService
{
    ...
    private async Task SweepAsync(CancellationToken ct)
    {
        ...
        if (eventsDeleted > 0 || deliveriesDeleted > 0 || auditDeleted > 0)
        {
            logger.LogInformation(
                "Retention sweep deleted {Events} events, {Deliveries} webhook deliveries, {Audit} audit log entries",
                eventsDeleted, deliveriesDeleted, auditDeleted);
        }
    }
}
